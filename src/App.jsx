...
function ResultStrip() {
  return <section className="result-strip" aria-label="真实成果">{metrics.map(item => <article key={item.label}><strong>{item.value}</strong><span>{item.label}</span></article>)}</section>;
}

function FlagshipCard({ project }) {
  return <article className="project-card"><a className="project-image" href={`#/project/${project.slug}`}><img src={project.screenshots[0].src} alt={project.screenshots[0].alt}/><span>查看案例 <Icon name="arrow"/></span></a><div className="project-copy"><div className="project-kicker"><span>{project.number}</span><i/>{project.category}</div><h3>{project.title}</h3><p className="project-statement">{project.statement}</p><p>{project.summary}</p><div className="proof-row">{project.evidence.slice(0,3).map(item => <span key={item.label}><b>{item.value}</b>{item.label}</span>)}</div><div className="project-actions"><a href={`#/project/${project.slug}`}>完整 Case Study <Icon name="arrow"/></a><a href={project.demo.href}>交互 Demo</a></div></div></article>;
}

function Work() {
  const flagship = projects.filter(item => item.flagship);
  const more = projects.filter(item => !item.flagship);
  return <section className="work" id="work"><SectionTitle index="01" title="用作品证明，而不是用标签描述。" text="三个旗舰项目都提供真实界面、我的角色、可核验依据和交互 Demo。"/><div className="project-list">{flagship.map(project => <FlagshipCard key={project.slug} project={project}/>)}</div><div className="more-projects"><h3>其他产品探索</h3><div>{more.map(project => <a key={project.slug} href={`#/project/${project.slug}`}><span>{project.number}</span><div><small>{project.category}</small><h4>{project.title}</h4><p>{project.summary}</p></div><Icon name="arrow"/></a>)}</div></div></section>;
}

function Method() {
  return <section className="method"><SectionTitle index="02" title="从业务问题到可验证结果。" text="模型只是系统的一层。产品还需要数据边界、人工确认、工具执行和结果反馈。"/><div className="method-grid">{capabilities.map(item => <article key={item.index}><span>{item.index}</span><h3>{item.title}</h3><p>{item.text}</p></article>)}</div></section>;
}

function About() {
  return <section className="about" id="about"><SectionTitle index="03" title="关于 Manny"/><div className="about-grid"><h2>在 AI 与真实工作<br/>之间搭一座桥。</h2><div><p className="lead">我是一名 <strong>AI Product Builder</strong>，关注 AI Agent、业务自动化、企业智能工作流和长期记忆系统。</p><p>我的项目来自招聘、企业评估、内容运营、桌面工具与个人效率场景。我习惯先理解重复劳动、信息断点和决策边界，再决定使用模型、规则、工具调用还是人工确认。</p><p>目标不是制造一个更会聊天的界面，而是做出可以进入流程、被使用、被复查并产生真实结果的产品。</p></div></div></section>;
}
...
function Home() {
  return <><Header/><main><Hero/><ResultStrip/><Work/><Method/><About/><Contact/></main><Footer/></>;
}
